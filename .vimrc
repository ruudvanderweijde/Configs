...
" as provided by Meik "fuller" TeÃŸmer
...
function! CommentLine()
  ...
  if file_name =~ '\.cpp$' || file_name =~ '\.hpp$' || file_name =~ '\.java$' || file_name =~ '\.php[2345]\?$' || file_name =~ '\.C$'
    " edit by Ruud, orig: execute \":silent! normal ^i//\<ESC>==\<down>^\"
  ...
  elseif file_name =~ '\.c$' || file_name =~ '\.h$' || file_name =~ '\.pc$' || file_name =~ '\.css$' || file_name =~ '\.js$'
    ...
    elseif stridx(getline("."), "\/\*") == -1 && stridx(getline("."), "\*\/") == -1
      " edit by Ruud, orig -> execute \":silent! normal ^i/*\<ESC>$a*/\<ESC>==\<down>^\"
...
function! UnCommentLine()
  ...
  " for .cpp or .hpp or .java or .C files use //
  if file_name =~ '\.cpp$' || file_name =~ '\.hpp$' || file_name =~ '\.java$' || file_name =~ '\.php[2345]\?$' || file_name =~ '\.C$'
    " edit by Ruud, orig -> execute \":silent! normal :nohlsearch\<CR>:s/\\/\\///\<CR>:nohlsearch\<CR>==\<up>^\"
    execute ":silent! normal :s/^\/\///\<ESC>\<up>^"
  " for .ml or .mli
  elseif file_name =~ '\.ml$' || file_name =~ '\.mli$'
